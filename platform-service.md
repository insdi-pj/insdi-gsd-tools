...
- **Confirm content fields use typed commons models from M1 P1**, per principles doc §4: `Organisation.submitter_allowlist: commons.schemas.SubmitterAllowlist | None`, `Organisation.auth_required_floor: commons.schemas.AuthFloor | None`, `Organisation.rate_limit_overrides: commons.schemas.RateLimitOverrides | None`. Same applies to `Workspace.submitter_allowlist`, etc. AI clients calling `organisations.create` then see allowed allowlist shapes (public / domain / explicit / tag) and their internal structure in `inputSchema`.
...
- **Drafting tools (M6+):** Platform has fewer obvious natural-language → input use cases than Gather/Verify/Calculate, but two candidates worth keeping on the roadmap: `organisations.draft_policies_from_description` (given a description of an Org's regulatory and security posture, propose `submitter_allowlist`, `auth_required_floor`, `domain_restriction_enabled`, federation config — to help during onboarding) and `federated_idps.draft_from_metadata` (given SAML metadata XML, propose the attribute mappings and config). Both follow the same progression as Gather/Verify/Calculate drafting tools — typed input models proven first, corpus and evaluation second, LLM-backed tool third. See `INSDI_GSD_PRINCIPLES.md` §4.
