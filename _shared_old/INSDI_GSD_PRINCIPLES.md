...
- **MCP parity from day one.** Every REST route added in any phase ships with its matching MCP tool in the same PR. Streamable HTTP transport, co-located in the same FastAPI app at `/mcp`. See §3 below — this is non-negotiable and is the most important departure from "defer complexity": MCP is a *surface convention*, not a *complexity layer*, because §8.4 §1.4 / §9.3 mandate "one state machine, two surfaces" enforced structurally. Retrofitting MCP later means rewriting every handler — exactly the failure mode the layered-complexity discipline exists to prevent.

**What is explicitly deferred:**
- Cognito and JWT validation → M2 (MCP tools in M1 are unauthenticated alongside REST routes)
- `Idempotency-Key` enforcement → M2 (header may be accepted but ignored in M1; if accepted, the validation message in the OpenAPI spec should mark it as "ignored in M1"). MCP idempotency auto-key (§8.4 §9.5) also deferred to M2.
- `ETag` / `If-Match` enforcement → M2
- Rate limiting → M2
- `_deprecation` envelope → not relevant until M2+ (no deprecated versions yet)
- Audit emission → M3
- **MCP-specific concerns deferred to M4+:** OAuth at MCP session start, the three-server-per-audience split (`mcp.insdi.com/{admin,home,public}`), `/.well-known/mcp` discovery, MCP session state in DynamoDB, build-time manifest assembly, `prompts` and `resources` content. M1–M3 ship MCP as a single streamable-HTTP endpoint at `/mcp` on the service Lambda's API, exposing tools only.
...
- The `X-Debug-Principal-Id` header is removed; principals now come from validated JWTs. The MCP `INSDI_DEBUG_PRINCIPAL_ID` env var is removed; MCP sessions authenticate via OAuth at session start.
- DB hydration of UserContext (Memberships, WorkspaceMemberships)
- Per-route auth dependency declarations on every existing route — and **per-tool auth on every existing MCP tool** (same `UserContext` resolution, surface-adapted)
- `Idempotency-Key` enforcement on REST per §8.4 §6.1–6.3 (24h retention, partitioned by `org_id`); MCP auto-derives idempotency key from `(session_id, tool_name, input_hash)` per §8.4 §9.5
- `ETag` on single-resource GETs; `If-Match` required on PATCH/PUT and on DELETE of high-value resources per §8.4 §6.4–6.6. MCP `.update` and `.delete` tools require equivalent — the tool accepts an `if_match` input parameter checked at use-case-function entry.
- Rate-limit response headers (`X-RateLimit-Limit`, `X-RateLimit-Remaining`, `X-RateLimit-Reset`, `X-RateLimit-Bucket`) per §8.4 §7.5; per-Org and per-IP buckets, DynamoDB-backed; **rate limits apply to MCP tool calls equally** (same bucket per Org)
...
- MCP-specific production concerns: split into per-audience servers, `/.well-known/mcp` discovery, MCP session state in DynamoDB, build-time manifest assembly — still M4+ (M1–M3 ship a single `/mcp` endpoint per service Lambda)
- Federation, agent grants (later)
- RLS (deferred until per-service schemas, §8.2 §1.4)

### M3 — Audit emission and observability

**Tier delivered:** Every write emits an audit event per §8.2 §5. Tier C reads emit operational telemetry. Request IDs propagate end-to-end across both surfaces.

**What M3 adds:**
- `audit.emit_with_tx()` for transactional audits (Tier A/B writes coupled to PG changes) — emitted from use-case functions, so REST and MCP both produce identical audit events for the same operation
...
- `X-Request-Id` header set on every REST response; equivalent `_request_id` field on MCP tool result envelopes; same value correlates across both surfaces, logs, and audit events
- Audit `actor.source` field distinguishes REST vs MCP origin (`rest` or `mcp`), but `actor.id` and `actor.type` are surface-agnostic
...
**Tier delivered:** RDS Proxy, KMS, observability hooks, OpenAPI assembly pipeline contributions, MCP-specific production architecture, deployment to staging.
...
- **Per-audience MCP server split** per §8.4 §9.1 — each service Lambda's MCP fragment is assembled into `mcp.insdi.com/{admin,home,public}` canonical servers by the build-time manifest assembler
- **MCP session state in DynamoDB** per §8.4 §9.9 — owned by Platform but used by all services via commons helpers
- **`/.well-known/mcp` discovery endpoint** per §8.4 §10 — served by Platform
- **OAuth at MCP session start** per §8.4 §8 — handled by Platform's OAuth endpoints
...
Each service has its own M5+ shape — federation, agent grants, link-scoped JWTs, advanced policy resolution, file uploads, flow orchestration, computation engines, MCP prompts and resources content. These are spelled out in each service's brief. MCP tools for these features are added in the same milestone the features land — never as a separate retrofit.

---

## 3. MCP parity — surface convention, not complexity layer

§8.4 §1.4 and §9.3 mandate that REST and MCP are "two surfaces over one state machine," **structurally enforced** by co-locating both in the same service Lambda and routing both through the same use-case functions. insdi is an AI-native platform — its API is *equally* a contract for humans (REST) and for AI agents (MCP). Treating MCP as a "feature to add later" inverts this and produces exactly the cross-surface drift §8.4 warns against.

**Therefore: MCP parity is a surface convention. It belongs alongside URL versioning, response shape, and error envelope — non-negotiable from M1 P1, layered just like REST.**

### 3.1 What MCP-parity means in each milestone

- **M1:** Every REST route ships with its matching MCP tool in the same phase, in the same PR, using the same Pydantic input schema. No auth on either surface. Streamable HTTP transport. Tools enumerable via MCP Inspector connecting to `http://localhost:<port>/mcp`.
- **M2:** When `RequireAdmin` (etc.) wires onto REST routes, the same `UserContext` resolution happens on MCP tool calls. Idempotency-Key required on REST creates; MCP auto-derives the equivalent from `(session_id, tool_name, input_hash)` per §8.4 §9.5. Rate-limit middleware applies equally to MCP tool calls.
- **M3:** Audit emission fires the same way regardless of surface (`audit_actor` field on `UserContext` doesn't care whether the request came from REST or MCP). `X-Request-Id` correlates across both surfaces — for MCP, the equivalent is a `_request_id` field on tool result envelopes or carried in trace context.
- **M4+:** MCP gets its own production surface: split into per-audience servers (`mcp.insdi.com/{admin,home,public}`), OAuth at session start, `/.well-known/mcp` discovery, MCP session state in DynamoDB, build-time manifest assembly. These are MCP-specific concerns that don't apply per-tool — they're the equivalent of "the OpenAPI assembly pipeline" for REST.
- **M5+:** Prompts (`/admin` v1 prompt set), Resources (docs, policy summaries, catalogues), MCP-content source-of-truth (open question OQ-A from §8.4).

### 3.2 What MCP tools look like in M1

For each entity in a service, the canonical tool set (CRUDLS convention):
...
<resource>.search     → POST   /v1/admin/<resource>/search   (only where REST has /search)
```

**CRUDLS** = Create, Read, Update, Delete, List, Search. The MCP convention is `read` (not `get`) per project decision. The `noun.verb` ordering follows §8.4 §9.4 (deviation from MCP-community `verb_noun` — flagged in tool descriptions).

Actions on resources become tools with the action as the verb:
...
Note: MCP tool names use `_` not `-` (per MCP convention for identifier safety): `flow_runs.advance`, not `flow-runs.advance`. The REST path stays kebab-case; the MCP tool name is the snake_case equivalent. This is the one consistent translation: **kebab-case URL resource → snake_case MCP namespace**.

### 3.3 How REST and MCP share code (the structural enforcement)

Every route handler and every MCP tool delegate to the **same use-case function** in `services/`:

```python
# services/templates.py
def create_template(ctx: UserContext, input: TemplateCreateInput) -> TemplateResponse:
    # All business logic lives here:
    # - workspace-membership check (M2+)
    # - policy validation
    # - DB write
    # - audit emission (M3+)
    # - event publication (M5+)
    ...

# api/v1/admin/templates.py (REST handler)
...
# mcp/tools/templates.py (MCP tool)
@mcp_tool("templates.create")
def create_template_tool(
    input: TemplateCreateInput,
    ctx: UserContext = mcp.context.UserContext,
) -> TemplateResponse:
    return services.templates.create_template(ctx, input)
```

Both sides are thin shape-adapters. The same Pydantic models are used as both REST request bodies and MCP tool input schemas — MCP's `inputSchema` is generated from the Pydantic model. This means a new field on `TemplateCreateInput` propagates to both surfaces automatically; you cannot add a field to one without the other.

**The rule:** if a new use-case function is added to `services/`, both a REST route and an MCP tool are added that call it. If a use-case function is updated (new field, new error case), both surfaces' tests are updated. PRs that touch only one surface for a change that should be both are rejected during `/gsd-verify-work`.

### 3.4 Local MCP testing — MCP Inspector

Every service in M1 must be testable with MCP Inspector locally:

```bash
# In the service repo, start the service (FastAPI on port 8000 by default)
uvicorn <service_name>.main:app --reload

# In a separate terminal, launch MCP Inspector against the running service
npx @modelcontextprotocol/inspector http://localhost:8000/mcp
```

MCP Inspector connects via streamable HTTP, enumerates tools via `tools/list`, and provides an interactive UI for calling each tool. M1's manual acceptance criteria (per each service brief) must include "exercise every tool via MCP Inspector" alongside "exercise every REST route via curl."

In M1, MCP Inspector authenticates the session by setting an env var when launching the server:

```bash
INSDI_DEBUG_PRINCIPAL_ID=au_dev_1 uvicorn <service_name>.main:app --reload
```

The server uses this env var to populate the `UserContext` for every MCP tool call. (REST routes still use the `X-Debug-Principal-Id` header per request; MCP uses the env var since session-level auth is the MCP-native pattern.) M2 replaces both with real auth.

### 3.5 Phase decomposition with MCP parity

A phase that adds an entity's CRUDLS now adds:

- ORM + migration (one set of files)
- Pydantic schemas — Create, Update, Response (one set of files)
- Use-case functions in `services/` (one file)
- REST handlers in `api/v1/admin/` (one file)
- MCP tools in `mcp/tools/` (one file)
- Tests covering use-case functions, REST handlers, MCP tools (one set, parallel structure)

The phase doesn't ship until **both surfaces work, both have tests passing, and MCP Inspector successfully exercises every tool**. A REST-only phase is incomplete, period.

### 3.6 Surface-specific concerns that aren't parity violations

A few things legitimately differ between REST and MCP — these aren't parity violations, they're the deliberate departures listed in §8.4 §9.5:

- **Error mapping:** REST returns RFC 9457 `application/problem+json`. MCP returns the same problem+json body inside an `isError: true` content block. The error builder produces one shape; surface adapters wrap it differently.
- **Idempotency:** REST requires client-provided `Idempotency-Key`; MCP auto-derives `(session_id, tool_name, input_hash)`. (M2+ concern.)
- **Authentication:** REST is per-request bearer; MCP is session-level OAuth. (M2+ concern.)
- **Pagination:** REST has `?cursor=...&limit=...` query params and `{ items, page }` response envelope. MCP tools have `cursor` and `limit` as input parameters and return the same envelope shape.
...
A reasonable phase size: one entity's CRUDLS + a meaningful subset of its routes, OR one cross-cutting concern (e.g. "wire Cognito JWT validation on existing routes and MCP tools") applied across the service.

**A phase is too big if** it cannot be planned in one `/gsd-plan-phase` pass, or if its execution would touch more than ~6-8 files significantly. Split it. (Note: an entity's CRUDLS phase touches ORM + schemas + use-cases + REST handlers + MCP tools + tests — that's a deliberately bigger surface than the same phase would be without MCP, but each of those is a thin file. Plan accordingly.)
